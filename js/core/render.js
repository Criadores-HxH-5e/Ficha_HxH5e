    ...
    // Roll Popup Overlay (O restante do seu código continua igual...)
    if (state.rollResult) {
        // ... (seu código do modal de rolagem)
    }
    ...
    // Modo leitura para Mestre: bloqueia interação sem impedir scroll
    if (state.readOnly && state.view === 'SHEET') {
        // Torna o conteúdo da sheet não-interativo (inert = sem cliques, sem foco, mas scroll ok)
        const sheetRoot = app.firstElementChild;
        if (sheetRoot) sheetRoot.setAttribute('inert', '');

        // Banner fixo com botão de voltar (fora do elemento inert)
        ...
        banner.innerHTML = `
            <span style="font-size:9px;font-weight:900;color:#e9d5ff;text-transform:uppercase;letter-spacing:.15em;font-family:'Orbitron',sans-serif">⚔️ Leitura — ${state.viewingUser?.username || ''}</span>
            <button id="readonly-back-btn" style="font-size:9px;font-weight:900;color:#e9d5ff;background:#6d28d9;border:1px solid #a78bfa;border-radius:6px;padding:4px 12px;cursor:pointer;font-family:'Orbitron',sans-serif">← Voltar</button>
        `;
