        function renderCreator(container) {
             if (!state.tempChar) { state.tempChar = { name: '', class: SYSTEM_DB.classes[0].id, race: 'Humano Comum', attributes: { FOR:10, DES:10, CON:10, INT:10, SAB:10, PRE:10 }, skills: [], otherSkills: [], raceTraits: [], traitDetails: {}, background: null, backgroundFeature: null, equipChoices: {}, inclinations: { positive: [], negative: [] }, fagogenese: null, effortRace: null, effortTrait: null, fqPreset: null, fqTamanho: null, fqPequenoPenalidade: null, fqInsetoDeslocamento: null }; }
            ...
            if (step === 0) {
                ...
                // Usando a nova função refatorada renderNenHexagon
                const diagramHtml = renderNenHexagon(state.tempChar.class, true);

                // Afinidade e Genialidade já rolados?
                const afinidade = state.tempChar.afinidade;
                const genialidade = state.tempChar.genialidade;

                const afinidadeHtml = afinidade != null ? (() => {
                    const nivel = afinidade <= 20 ? { label:'Baixa', cor:'#6b7280', desc:'Seu Nen é bruto mas eficiente' }
                        : afinidade <= 50 ? { label:'Média', cor:'#fbbf24', desc:'Equilíbrio entre poder e controle' }
                        : afinidade <= 80 ? { label:'Alta', cor:'#4ade80', desc:'Seu Nen flui com naturalidade' }
                        : { label:'Excepcional', cor:'#00f0ff', desc:'Você nasceu para dominar o Nen' };
                    return `<div style="background:#0a0f1a;border:2px solid ${nivel.cor}44;border-radius:12px;padding:14px;text-align:center">
                        <div style="font-size:8px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:2px;margin-bottom:4px">🎯 Afinidade com Nen</div>
                ...
                const genialidadeHtml = genialidade != null ? (() => {
                 const tiers = {
                        'Normal':     {
                            cor:'#9ca3af', icon:'⚔️', raridade:'~78.5%',
                            titulo:'Normal',
                            beneficio:'Segue a evolução padrão do sistema.',
                            flavor:'Cada Hunter começa de algum lugar. A consistência e o esforço moldam quem você se torna.'
                        },
                        'Talentoso': {
                            cor:'#60a5fa', icon:'💠', raridade:'~18.5%',
                            titulo:'Talentoso',
                            beneficio:'+2 Graus de Potência para distribuir na criação do Primeiro Hatsu.',
                            flavor:'Um lampejo acima da média. Poucos percebem no início — mas os Mestres notam.'
                        },
                        'Gênio':     {
                            cor:'#fbbf24', icon:'✨', raridade:'~2.5%',
                            titulo:'Gênio',
                            beneficio:'Recebe XP 1,5× maior em qualquer situação.',
                            flavor:'Raro. Impressionante. O tipo que faz Mestres pararem para observar uma segunda vez.'
                        },
                        'Ultimate':  {
                            cor:'#ff4df7', icon:'👑', raridade:'~0.25%',
                            titulo:'Ultimate',
                            beneficio:'XP dobrado + +5 Graus de Potência para distribuir livremente ao longo da evolução.',
                            flavor:'Anormalmente superior. Ocorre menos de uma vez a cada 400 despertar de NEN. Sua lenda começa agora.'
                        }
                    };
                    ...
                    return `<div style="background:#0d1117;border:2px solid ${gn.cor}88;border-radius:16px;padding:16px;text-align:center;box-shadow:0 0 20px ${gn.cor}22">
                        <div style="font-size:8px;font-weight:900;color:#9ca3af;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px">🎯 Talento para Proficiência de NEN</div>
                        ...
                        <div style="margin-top:12px;background:#060d1a;border:1px solid ${gn.cor}55;border-radius:10px;padding:12px;text-align:left">
                            ${isSpecial ? `<div style="font-size:8px;font-weight:900;color:${gn.cor};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px">⚡ BENEFÍCIO</div>` : ''}
                        ...
                        <div style="margin-top:10px;font-size:8px;color:#6b7280;line-height:1.6;padding:0 4px">
                            A proficiência inata representa o <em style="color:#9ca3af">fator de aprendizado natural</em>.<br>
                            A proficiência <em style="color:#9ca3af">real</em> se manifesta com uso criativo e consistente.
                        </div>
                    </div>`;
                })() : '';
                ...
                SYSTEM_DB.racas.forEach(r => {
                    ...
                    if (r.nome === 'Formiga Quimera') {
                         ...
                         // Seletor de Origem da Fagogênese
                         ...
                         const originHtml = `
                            <div class="mb-4" onclick="event.stopPropagation()">
                                <h4 class="text-[10px] font-black text-neon-theme uppercase tracking-widest mb-2">Origem da Fagogênese (Visual Predominante)</h4>
                                <div class="bg-gray-950/50 rounded-lg p-2 border border-gray-800/50">
                                    <select onchange="selectFagogenese(this.value)" class="w-full bg-black border border-gray-700 rounded p-2 text-xs text-white outline-none focus:border-neon-theme">
                         ...
                         // rev. Manual (Bestiário) — referência pronta de F.Q. dentro da categoria escolhida:
                         // preenche deslocamento e sugere (pré-marca) 1-2 características, sem travar a escolha.
                         const presetsList = (SYSTEM_DB.fqPresets && SYSTEM_DB.fqPresets[currentOrigin]) || [];
                         const currentPreset = state.tempChar.fqPreset || '';
                         const presetHtml = presetsList.length > 0 ? `
                            <div class="mb-4" onclick="event.stopPropagation()">
                                <h4 class="text-[10px] font-black text-neon-theme uppercase tracking-widest mb-2">Referência Pronta (opcional)</h4>
                                <p class="text-[9px] text-gray-500 leading-tight mb-2">Baseada em uma besta comum do livro de monstros — preenche o deslocamento e sugere características (você pode trocar depois).</p>
                                <div class="bg-gray-950/50 rounded-lg p-2 border border-gray-800/50">
                                    <select onchange="selectFQPreset(this.value)" class="w-full bg-black border border-gray-700 rounded p-2 text-xs text-white outline-none focus:border-neon-theme">
                                        <option value="">— Criar do zero —</option>
                                        ${presetsList.map(p => `<option value="${p.nome}" ${currentPreset === p.nome ? 'selected' : ''}>${p.nome} (${p.desloc})</option>`).join('')}
                                    </select>
                                </div>
                            </div>` : '';

                         // rev. Manual — Insetos/Insectóides escolhem entre Voo ou Escalada quando não há
                         // referência pronta selecionada (a referência já traz o tipo certo embutido).
                         const insetoDeslocHtml = (currentOrigin === 'Inseto/Insectóide' && !currentPreset) ? `
                            <div class="mb-4" onclick="event.stopPropagation()">
                                <h4 class="text-[10px] font-black text-neon-theme uppercase tracking-widest mb-2">Deslocamento Especial de Inseto</h4>
                                <div class="flex gap-2">
                                    ${['voo', 'escalada'].map(tipo => `<button onclick="selectFQInsetoDeslocamento('${tipo}')" class="flex-1 py-2 rounded-lg border text-[10px] font-bold uppercase tracking-widest transition-all ${((state.tempChar.fqInsetoDeslocamento || 'voo') === tipo) ? 'bg-neon-theme/10 border-neon-theme text-neon-theme' : 'bg-gray-900 border-gray-800 text-gray-500'}">${tipo === 'voo' ? '🕊️ Voo' : '🧗 Escalada'}</button>`).join('')}
                                </div>
                            </div>` : '';

                         // rev. Manual (regra de tamanho) — Miúdo/Pequeno alteram atributos; Médio é o padrão.
                         const currentTamanho = state.tempChar.fqTamanho || 'Médio';
                         const tamanhoHtml = `
                            <div class="mb-4" onclick="event.stopPropagation()">
                                <h4 class="text-[10px] font-black text-neon-theme uppercase tracking-widest mb-2">Tamanho</h4>
                                <div class="grid grid-cols-3 gap-2">
                                    ${['Miúdo', 'Pequeno', 'Médio'].map(t => `<button onclick="selectFQTamanho('${t}')" class="py-2 rounded-lg border text-[10px] font-bold uppercase tracking-widest transition-all ${currentTamanho === t ? 'bg-neon-theme/10 border-neon-theme text-neon-theme' : 'bg-gray-900 border-gray-800 text-gray-500'}">${t}</button>`).join('')}
                                </div>
                                <p class="text-[9px] text-gray-500 leading-tight mt-2">${currentTamanho === 'Miúdo' ? '+2 DES, -1 FOR e -1 CON' : currentTamanho === 'Pequeno' ? '+1 DES e -1 em FOR ou CON (escolha abaixo)' : 'Sem alteração de atributos.'}</p>
                                ${currentTamanho === 'Pequeno' ? `<div class="flex gap-2 mt-2">${['FOR', 'CON'].map(a => `<button onclick="selectFQPequenoPenalidade('${a}')" class="flex-1 py-1.5 rounded-lg border text-[10px] font-bold uppercase tracking-widest transition-all ${(state.tempChar.fqPequenoPenalidade || 'FOR') === a ? 'bg-red-500/10 border-red-500 text-red-400' : 'bg-gray-900 border-gray-800 text-gray-500'}">-1 ${a}</button>`).join('')}</div>` : ''}
                            </div>`;

                         featuresHtml = originHtml + presetHtml + insetoDeslocHtml + tamanhoHtml + `<div class="mb-4"><h4 class="text-[10px] font-black text-neon-theme uppercase tracking-widest mb-2 flex justify-between">Características da Espécie <span id="trait-counter" class="${selectedCount >= maxTraits ? 'text-neon-theme' : 'text-gray-500'}">(${selectedCount}/${maxTraits})</span></h4><div class="bg-gray-950/50 rounded-lg p-2 border border-gray-800/50 grid grid-cols-1 gap-2">${r.caracteristicas.map(f => {
                    ...
                    } else if (r.nome === 'Humano Comum') {
                        // rev. Manual (Esforço no lugar de talento) — o Humano abre mão de nada e escolhe
                        ...
                        featuresHtml = `<div class="mb-4" onclick="event.stopPropagation()">
                            ...
                            <div class="space-y-2">
                                ${esforcoData.opcoes.map(o => `<label class="flex items-start gap-3 cursor-pointer p-2 rounded hover:bg-white/5 transition-colors bg-gray-950/50 border border-gray-800/50"><div class="relative flex items-center mt-0.5"><input type="radio" name="effort-trait" value="${o.nome}" class="peer sr-only" onclick="selectEffortTrait('${o.nome}')" ${state.tempChar.effortTrait === o.nome ? 'checked' : ''}><div class="w-4 h-4 border-2 border-gray-600 rounded-full peer-checked:border-neon-theme peer-checked:bg-neon-theme/20 transition-all flex items-center justify-center"><div class="w-2 h-2 rounded-full bg-transparent peer-checked:bg-neon-theme transition-colors radio-indicator"></div></div></div><div class="flex-1"><span class="text-xs font-bold text-white block ${state.tempChar.effortTrait === o.nome ? 'text-neon-theme' : ''}">${o.nome}</span><span class="text-[10px] text-gray-500 leading-tight block">${o.efeito}</span></div></label>`).join('')}
                            </div>
                        </div>`;
                    } else {
                        // Traços sempre concedidos (ex. Visão na Penumbra, Deslocamento) — só exibição.
                        const fixedFeatures = r.caracteristicas || [];
                        let fixedHtml = '';
                        if (fixedFeatures.length > 0) fixedHtml = `<div class="mb-3"><h4 class="text-[10px] font-black text-neon-theme uppercase tracking-widest mb-2">Características${(r.opcoes_caracteristica||[]).length > 0 ? ' (recebe todas)' : ''}</h4><div class="bg-gray-950/50 rounded-lg p-3 border border-gray-800/50">${fixedFeatures.map(f => `<div class="mb-2 last:mb-0"><span class="text-[11px] font-bold text-gray-300 block">${f.nome}</span><span class="text-[10px] text-gray-500 block leading-tight">${f.efeito || f.mecanica || ''} ${f.tabela_ancestral ? '('+f.tabela_ancestral+')' : ''}</span></div>`).join('')}</div></div>`;
                        // Traços de escolha (ex. "escolha uma característica abaixo") — rádio real, mesmo
                        // padrão de Antecedente/Esforço. Antes disso era só texto informativo, sem seleção.
                        const choiceFeatures = r.opcoes_caracteristica || [];
                        let choiceHtml = '';
                        if (choiceFeatures.length > 0) {
                            choiceHtml = `<div class="mb-4" onclick="event.stopPropagation()">
                                <h4 class="text-[10px] font-black text-neon-theme uppercase tracking-widest mb-2">Escolha uma Característica</h4>
                                <div class="space-y-2">
                                    ${choiceFeatures.map(f => `<label class="flex items-start gap-3 cursor-pointer p-2 rounded hover:bg-white/5 transition-colors bg-gray-950/50 border border-gray-800/50"><div class="relative flex items-center mt-0.5"><input type="radio" name="race-feature-choice" value="${f.nome}" class="peer sr-only" onclick="selectRaceFeatureChoice('${f.nome}')" ${state.tempChar.raceFeatureChoice === f.nome ? 'checked' : ''}><div class="w-4 h-4 border-2 border-gray-600 rounded-full peer-checked:border-neon-theme peer-checked:bg-neon-theme/20 transition-all flex items-center justify-center"><div class="w-2 h-2 rounded-full bg-transparent peer-checked:bg-neon-theme transition-colors radio-indicator"></div></div></div><div class="flex-1"><span class="text-xs font-bold text-white block ${state.tempChar.raceFeatureChoice === f.nome ? 'text-neon-theme' : ''}">${f.nome}</span><span class="text-[10px] text-gray-500 leading-tight block">${f.efeito || f.mecanica || ''}</span></div></label>`).join('')}
                                </div>
                            </div>`;
                        }
                        featuresHtml = fixedHtml + choiceHtml;
                    }
                ...
                SYSTEM_DB.antecedentes.forEach(bg => {
                    ...
                    const chevronClass = isSelected ? 'rotate-0' : '-rotate-90';
                    const pendingEquip = isSelected ? getPendingEquipSlots(bg) : [];
                    const equipRowsHtml = bg.equipamento.map((eqItem, eqIdx) => {
                        const eqInfo = classifyEquipSlot(eqItem);
                        if (isSelected && eqInfo.type !== 'fixed') {
                            const chosen = (state.tempChar.equipChoices || {})[eqIdx];
                            return `<div class="bg-gray-900 border border-neon-theme/40 px-2 py-1 rounded text-[10px] text-white font-mono flex items-center justify-between"><span>${chosen || eqItem}</span><span class="text-[8px] text-neon-theme uppercase font-bold">escolhido</span></div>`;
                        }
                    ...
                    if (bonusReq.type === 'wildcard') {
                        ...
                        <div style="background:#0d1421;border:2px solid var(--theme-color,#00ff9d);border-radius:16px;padding:16px;margin-bottom:16px">
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
                                <div>
                                    <div style="font-family:'Orbitron',sans-serif;font-weight:900;font-size:10px;color:var(--theme-color,#00ff9d);text-transform:uppercase;letter-spacing:2px">🎁 Bônus Racial</div>
                        ...
                        bonusHtml = `
                        <div style="background:#0d1421;border:2px solid var(--theme-color,#00ff9d);border-radius:16px;padding:16px;margin-bottom:16px">
                            <div style="font-family:'Orbitron',sans-serif;font-weight:900;font-size:10px;color:var(--theme-color,#00ff9d);text-transform:uppercase;letter-spacing:2px;margin-bottom:4px">🎁 Bônus Racial</div>
                ...
                contentHtml = `<div class="space-y-6"><div class="text-center"><h2 class="text-3xl font-display font-bold text-white mb-1">${state.tempChar.name || 'Sem Nome'}</h2><span class="bg-[${color}]/20 text-[${color}] px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest border border-[${color}]/30">${state.tempChar.class}</span></div>${bonusHtml}<div class="bg-gray-900 rounded-xl p-4 border border-gray-800 space-y-4"><div class="flex justify-between border-b border-gray-800 pb-2"><span class="text-xs text-gray-500 uppercase">Raça</span><span class="text-sm font-bold text-white">${state.tempChar.race}</span></div><div class="flex justify-between border-b border-gray-800 pb-2"><span class="text-xs text-gray-500 uppercase">Antecedente</span><span class="text-sm font-bold text-white">${state.tempChar.background || '-'}</span></div>${state.tempChar.raceTraits && state.tempChar.raceTraits.length > 0 ? `<div><span class="text-xs text-gray-500 uppercase block mb-2">Traços Raciais Selecionados</span><div class="flex flex-wrap gap-1">${state.tempChar.raceTraits.map(t => { 
                ...
                }).join('')}</div></div>` : ''}<div><span class="text-xs text-gray-500 uppercase block mb-2">Atributos Base</span><div class="grid grid-cols-6 gap-1 text-center">${Object.entries(state.tempChar.attributes).map(([k,v]) => `<div class="bg-black rounded p-1"><div class="text-[8px] text-gray-500">${k}</div><div class="text-xs font-bold text-white">${v}</div></div>`).join('')}</div></div><div><span class="text-xs text-gray-500 uppercase block mb-2">Treinamentos</span><div class="flex flex-wrap gap-1 mb-2">${state.tempChar.skills.length > 0 ? state.tempChar.skills.map(s => `<span class="text-[10px] bg-gray-800 text-gray-300 px-2 py-1 rounded border border-gray-700">${s}</span>`).join('') : '<span class="text-[10px] text-gray-600 italic">Nenhum treinamento principal</span>'}</div><div class="flex flex-wrap gap-1">${state.tempChar.otherSkills.length > 0 ? state.tempChar.otherSkills.map(s => `<span class="text-[10px] bg-gray-800 text-neon-blue/70 px-2 py-1 rounded border border-gray-700">${s}</span>`).join('') : '<span class="text-[10px] text-gray-600 italic">Nenhum treinamento extra</span>'}</div></div><div><span class="text-xs text-gray-500 uppercase block mb-2">Inclinações</span><div class="space-y-1">${state.tempChar.inclinations.positive.map(i => `<div class="text-[10px] text-neon-green flex justify-between"><span>+ ${i.nome}</span><span>${i.custo}</span></div>`).join('')}${state.tempChar.inclinations.negative.map(i => `<div class="text-[10px] text-neon-red flex justify-between"><span>- ${i.nome}</span><span>${i.valor}</span></div>`).join('')}</div></div></div>${!isBonusReady ? `<div class="bg-red-500/10 border border-red-500/50 p-3 rounded-xl flex items-center gap-3 text-red-400 text-xs font-bold animate-pulse"><i data-lucide="alert-circle" size="20"></i> Aloque todos os pontos de bônus para finalizar.</div>` : ''}</div>`;
            }
