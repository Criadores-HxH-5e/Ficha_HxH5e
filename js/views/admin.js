        function renderAdmin(container) {
                    ...
                    <div style="display:flex;align-items:center;gap:10px">
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div style="display:flex;gap:4px;margin-top:4px;flex-wrap:wrap">
                                ${isAdm ? `<span style="font-size:8px;background:#dc262633;color:#f87171;border-radius:4px;padding:2px 6px;font-weight:900">ðŸ›¡ ADMIN${isHardcoded?' (fixo)':''}</span>` : ''}
                                ${isMst ? `<span style="font-size:8px;background:#7c3aed33;color:#a78bfa;border-radius:4px;padding:2px 6px;font-weight:900">âš”ï¸ MESTRE</span>` : ''}
                            </div>
                        </div>
                        <div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0">
                            ...
                            ${!isHardcoded && isAdm ? `<button onclick="adminRevokeAdmin('${u.id}')" style="padding:5px 8px;border-radius:7px;background:#3b0f0f;border:1px solid #7f1d1d;color:#f87171;font-size:8px;font-weight:900;text-transform:uppercase;cursor:pointer;font-family:'Orbitron',sans-serif">â€“ Admin</button>` : ''}
                ...
                <div style="background:#0a0a0f;min-height:100%;display:flex;flex-direction:column">
                    <div style="padding:16px;border-bottom:1px solid #1f2937;display:flex;align-items:center;gap:12px;flex-shrink:0">
                        ...
                        <div>
                            ...
                            <div style="font-size:16px;font-weight:900;color:#fff;font-family:'Orbitron',sans-serif">ADMINISTRAÃ‡ÃƒO</div>
                        </div>
                    ...
                    <div style="flex:1;overflow-y:auto;padding:16px">
                        ...
                        <div style="background:#111827;border:1px solid #1f2937;border-radius:12px;padding:14px;margin-bottom:16px">
                            <div style="font-size:9px;color:#f87171;font-weight:900;text-transform:uppercase;letter-spacing:.15em;margin-bottom:10px">ðŸ›¡ Adicionar Admin por ID</div>
                        ...
                        <div style="font-size:9px;color:#6b7280;font-weight:900;text-transform:uppercase;letter-spacing:.15em;margin-bottom:12px">UsuÃ¡rios Registrados (${users.length})</div>
                        ${users.length === 0 ? '<div style="text-align:center;color:#4b5563;font-style:italic;padding:32px 0;border:2px dashed #1f2937;border-radius:12px;font-size:11px">Nenhum usuÃ¡rio registrado ainda.</div>' : userRows}
                    </div>
                </div>
            ...
            window._addAdminById = async function() {
                ...
                if (!targetId || !/^\d{17,20}$/.test(targetId)) {
                    alert('ID invÃ¡lido. Digite um Discord User ID vÃ¡lido (17-20 dÃ­gitos numÃ©ricos).');
                    return;
                }
                if (ADMIN_USERS.includes(targetId)) { alert('Este usuÃ¡rio jÃ¡ Ã© admin fixo (hardcoded).'); return; }
                const existing = await sbSelect('admins', `user_id=eq.${targetId}&select=user_id`);
                if (existing && existing.length > 0) { alert('Este ID jÃ¡ Ã© admin.'); return; }
