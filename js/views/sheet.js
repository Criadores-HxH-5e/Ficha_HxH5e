        function renderSheet(container) {
            ...
            // Helpers de renderizaÃ§Ã£o interna da ficha
            ...
            if (state.activeTab === 'FICHA') {
                const ATTR_FULL_NAMES = { 'FOR': 'FORÃ‡A', 'DES': 'DESTREZA', 'CON': 'CONSTITUIÃ‡ÃƒO', 'INT': 'INTELIGÃŠNCIA', 'SAB': 'SABEDORIA', 'PRE': 'PRESENÃ‡A', 'CAR': 'PRESENÃ‡A' };
                ...
                const infoGridHtml = `<div class="grid grid-cols-4 divide-x divide-gray-800 bg-[#0b0c10] border-t border-b border-gray-800 mb-2"><div class="p-2 flex flex-col items-center justify-center"><span class="text-[9px] font-bold text-neon-blue uppercase tracking-widest mb-0.5">TendÃªncia</span><select onchange="updateCharProperty('alignment', this.value)" class="bg-transparent text-white font-display font-bold text-xs uppercase outline-none cursor-pointer text-center appearance-none w-full"><option value="HerÃ³ico" ${char.alignment === 'HerÃ³ico' ? 'selected' : ''} class="bg-gray-900">HerÃ³ico</option><option value="CaÃ³tico" ${char.alignment === 'CaÃ³tico' ? 'selected' : ''} class="bg-gray-900">CaÃ³tico</option><option value="Neutro" ${!char.alignment || char.alignment === 'Neutro' ? 'selected' : ''} class="bg-gray-900">Neutro</option><option value="Maligno" ${char.alignment === 'Maligno' ? 'selected' : ''} class="bg-gray-900">Maligno</option></select></div><div class="p-2 flex flex-col items-center justify-center"><span class="text-[9px] font-bold text-neon-yellow uppercase tracking-widest mb-0.5">ProficiÃªncia</span><span class="font-display font-bold text-lg text-white tracking-wider">+${getProficiencyBonus(char.level)}</span></div><div class="p-2 flex flex-col items-center justify-center"><span class="text-[9px] font-bold text-white uppercase tracking-widest mb-0.5">Desl.</span><span class="font-display font-bold text-lg text-white tracking-wider">${char.vitals.desl || 9}m</span></div><div class="p-2 flex flex-col items-center justify-center"><span class="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">Jogador</span><input type="text" value="${char.playerName || ''}" placeholder="Nome..." onchange="updateCharProperty('playerName', this.value)" class="bg-transparent text-white font-display font-bold text-xs outline-none w-full text-center placeholder-gray-700"></div></div>`;
                ...
                tabContent = `${infoGridHtml}${vitalsGridHtml}${(() => {
                    ...
                    const PRINCIPIOS = [
                        { key:'ten', label:'TEN', icon:'ðŸ›¡ï¸',
                          custo: (() => {
                              const isR = char.class === 'REFORÃ‡O' || char.class === 'INTENSIFICAÃ‡ÃƒO';
                              const nv = d.ten || 1;
                              if (isR) return nv === 2 ? 5 : 10;
                              return 20;
                          })(),
                          efeito: d.ten===1?'+2 RD':d.ten===2?'+4 RD':'+6 RD',
                          desc: 'ReaÃ§Ã£o â€” RD fÃ­sica', show: (d.ten||0)>0 },
                        { key:'ren', label:'REN', icon:'ðŸ’ª', custo:5,
                          efeito: '+1 Grau dano',
                          desc: 'AÃ§Ã£o BÃ´nus â€” aumenta grau', show: (d.ren||0)>0 },
                        { key:'zetsu', label:'ZETSU', icon:'ðŸ‘ï¸', custo:0,
                          efeito: d.zetsu===1?'+5% Aura/3rod':d.zetsu===2?'+10% Aura/2rod':'+10% Aura/1rod',
                          desc: 'AÃ§Ã£o BÃ´nus â€” recupera aura', show: (d.zetsu||0)>0 },
                        { key:'en', label:'EN', icon:'ðŸ”µ', custo:10,
                          efeito: 'Detecta 3m', desc: '10% + 2 ReaÃ§Ãµes', show: !!d.en },
                        { key:'inp', label:'IN', icon:'ðŸŒ‘', custo:5,
                          efeito: 'Oculta objeto', desc: '5% por rodada', show: !!d.inp },
                        { key:'gyo', label:'GYO', icon:'ðŸ”', custo:10,
                          efeito: 'Ver aura/oculto', desc: '10% olhos / 30% corpo', show: !!d.gyo },
                        { key:'shu', label:'SHU', icon:'âš¡', custo:10,
                          efeito: '+1d4 dano+CA', desc: '10% em objeto 1m', show: !!d.shu },
                        { key:'ken', label:'KEN', icon:'ðŸ°', custo:30,
                          efeito: 'CA Ã—2 por 2rod', desc: '30% + 4 ReaÃ§Ãµes', show: !!d.ken },
                        { key:'ko', label:'KO', icon:'ðŸ”¥', custo:30,
                          efeito: 'Dano Ã—3, CA âˆ’80%', desc: '30% Aura', show: !!d.ko },
                        { key:'ryu', label:'RYU', icon:'ðŸŒŠ', custo:30,
                    ...
                    return `<div style="padding:8px 12px;border-bottom:1px solid #111827;background:#0a0f1a">
                        <div style="font-size:7px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:2px;margin-bottom:6px">ðŸ”® PrincÃ­pios de NEN</div>
                        <div style="display:flex;gap:4px;flex-wrap:wrap">${btns}</div>
                    </div>`;
                })()}${rollModesHtml}${attributesHtml}${(() => {
                    // â”€â”€ SeÃ§Ã£o de Ataques â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    ...
                    const rows = attacksList.map(w => {
                        ...
                        return `<div style="display:flex;align-items:center;gap:8px;background:#0a0f1a;border:1px solid #1f2937;border-radius:10px;padding:8px 10px;margin-bottom:6px">
                            ...
                            <button onclick="openAttackModal('${safeNome}','${safeDano}','${safeTipo}')" style="flex-shrink:0;background:#ef444422;border:1px solid #ef444455;color:#f87171;border-radius:8px;padding:6px 10px;font-size:10px;font-weight:900;cursor:pointer;letter-spacing:.5px">âš”ï¸</button>
                        </div>`;
                    }).join('');
                    const weaponsSection = `<div style="margin:0 16px 20px"><div style="font-size:8px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px;border-top:1px solid #1f2937;padding-top:12px">âš”ï¸ Ataques</div>${rows}</div>`;
                    if (!state.attackModal) return weaponsSection;
                    // â”€â”€ Modal de escolha FOR/DES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                        ...
                        <div style="background:#0d1117;border:2px solid #ef4444;border-radius:16px;padding:20px;width:80%;max-width:300px;box-shadow:0 0 40px #ef444440" onclick="event.stopPropagation()">
                            ...
                            <div style="display:flex;flex-direction:column;gap:8px">
                                <button onclick="pickAttackAttr('${safeNome2}','${safeDano2}','FOR')" style="width:100%;padding:13px;border-radius:10px;background:#ef444422;border:1px solid #ef444466;color:#f87171;font-family:'Orbitron',sans-serif;font-weight:900;font-size:10px;cursor:pointer;text-transform:uppercase;letter-spacing:1px">
                                    ðŸ’ª FORÃ‡A (${forMod >= 0 ? '+' : ''}${forMod}) Â· Ataque ${(forMod + pb2) >= 0 ? '+' : ''}${forMod + pb2}
                                </button>
                                <button onclick="pickAttackAttr('${safeNome2}','${safeDano2}','DES')" style="width:100%;padding:13px;border-radius:10px;background:#38bdf822;border:1px solid #38bdf866;color:#38bdf8;font-family:'Orbitron',sans-serif;font-weight:900;font-size:10px;cursor:pointer;text-transform:uppercase;letter-spacing:1px">
                                    ðŸŒ¬ï¸ DESTREZA (${desMod >= 0 ? '+' : ''}${desMod}) Â· Ataque ${(desMod + pb2) >= 0 ? '+' : ''}${desMod + pb2}
                                </button>
                            </div>
                ...
                const sections = [ { id: 'personality', label: 'Personalidade', icon: 'fingerprint' }, { id: 'history', label: 'HistÃ³ria', icon: 'scroll' }, { id: 'organizations', label: 'OrganizaÃ§Ãµes', icon: 'building-2' }, { id: 'enemies', label: 'Inimigos', icon: 'skull' }, { id: 'allies', label: 'Aliados', icon: 'heart-handshake' } ];
                tabContent = `<div class="p-4 space-y-3"><div class="text-center mb-4"><h2 class="text-lg font-display font-bold text-white uppercase tracking-widest">Biografia</h2><p class="text-[10px] text-gray-500 uppercase tracking-widest font-bold">InformaÃ§Ãµes Detalhadas</p></div>${sections.map(section => { const isOpen = state.bioAccordionOpen === section.id; const isPersonality = section.id === 'personality'; let hasContent = false; if (isPersonality) { hasContent = (bioData.personality_traits || bioData.personality_goals || bioData.personality_hates || bioData.personality_likes || bioData.personality); } else { hasContent = bioData[section.id] && bioData[section.id].trim().length > 0; } const borderColor = isOpen ? `border-[${themeColor}]` : 'border-gray-800'; const headerColor = isOpen ? `text-[${themeColor}]` : (hasContent ? 'text-white' : 'text-gray-500'); const renderSubField = (key, label, placeholder, legacyVal = '') => `<div class="mb-3 last:mb-0 w-full"><label class="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-1 block pl-1">${label}</label><textarea class="w-full h-20 bg-gray-950 border border-gray-800 rounded-lg p-3 text-xs text-gray-300 focus:border-[${themeColor}] focus:shadow-[0_0_10px_rgba(var(--theme-rgb),0.1)] transition-all custom-scrollbar resize-none leading-relaxed" placeholder="${placeholder}" onblur="updateBio('${key}', this.value)">${bioData[key] || legacyVal || ''}</textarea></div>`; return `<div class="bg-gray-900 border ${borderColor} rounded-xl transition-all duration-300"><div onclick="toggleBioAccordion('${section.id}')" class="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-800/50 rounded-xl"><div class="flex items-center gap-3"><div class="bg-gray-950 p-2 rounded-lg border border-gray-800"><i data-lucide="${section.icon}" size="18" class="${headerColor}"></i></div><div><h3 class="font-display font-bold text-sm uppercase tracking-wider ${headerColor}">${section.label}</h3>${hasContent && !isOpen ? '<span class="text-[8px] text-gray-500 bg-gray-950 px-1.5 py-0.5 rounded border border-gray-800 font-bold uppercase tracking-widest">Preenchido</span>' : ''}</div></div><div class="transition-transform duration-300 ${isOpen ? 'rotate-180 text-white' : 'text-gray-600'}"><i data-lucide="chevron-down" size="18"></i></div></div><div class="accordion-content ${isOpen ? 'open' : ''}"><div class="px-4 pb-4"><div class="h-px w-full bg-gray-800 mb-3"></div>${isPersonality ? `${renderSubField('personality_traits', 'TraÃ§os de Personalidade', 'Descreva como seu personagem se comporta...', bioData.personality)}${renderSubField('personality_goals', 'Sonho / Objetivo', 'Qual o maior desejo do seu personagem?')}<div class="flex gap-3">${renderSubField('personality_hates', 'O que odeio', 'O que te tira do sÃ©rio...')}${renderSubField('personality_likes', 'O que gosto', 'O que te agrada...')}</div>` : `<textarea class="w-full h-32 bg-gray-950 border border-gray-800 rounded-lg p-3 text-xs text-gray-300 focus:border-[${themeColor}] focus:shadow-[0_0_10px_rgba(var(--theme-rgb),0.1)] transition-all custom-scrollbar resize-none leading-relaxed" placeholder="Escreva aqui..." onblur="updateBio('${section.id}', this.value)">${bioData[section.id] || ''}</textarea>`}</div></div></div>`; }).join('')}</div>`;
    ...
    const tipoIcons = new Proxy(_tipoIconsMap, { get(t,k) { if(k in t) return t[k]; const parts=(k||'').split('+'); return parts.map(p=>t[p]||'âœ¦').join(''); } });
                    hatsuListHtml = `<div style="width:100%;padding:0 4px;margin-bottom:16px">
                        <div style="font-size:9px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:2px;margin-bottom:10px;text-align:center">âš¡ Hatsus Criados</div>
                            ...
                            return `<div onclick="openHatsuDetail(${idx})" style="background:#0a0f1a;border:2px solid ${catColor}44;border-radius:14px;padding:14px;margin-bottom:10px;position:relative;cursor:pointer;transition:border-color .2s,box-shadow .2s" onmouseover="this.style.borderColor='${catColor}99';this.style.boxShadow='0 0 16px ${catColor}22'" onmouseout="this.style.borderColor='${catColor}44';this.style.boxShadow='none'">
                                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:8px">
                                    <div style="flex:1;min-width:0">
                                        ...
                                        <div style="font-size:8px;color:#4b5563;margin-top:3px">${tipoIcons[h.tipo]||'âœ¦'} ${h.tipo||'â€”'} Â· NÃ­vel ${h.nivel||'?'} Â· ${h.criadoEm||''}</div>
                                    </div>
                                    <button onclick="event.stopPropagation();deleteHatsuConfirm(${idx},'list')"
                                        style="flex-shrink:0;background:#1f2937;border:1px solid #374151;border-radius:8px;color:#6b7280;font-size:11px;padding:4px 8px;cursor:pointer;line-height:1">âœ•</button>
                                </div>
                    ...
                    <div class="p-4 h-full flex flex-col items-center" style="overflow-y:auto">
                        ...
                        ${(() => {
                            ...
                            const afNivel = af <= 20 ? { label:'Baixa', cor:'#6b7280' }
                                : af <= 50 ? { label:'MÃ©dia', cor:'#fbbf24' }
                                : af <= 80 ? { label:'Alta', cor:'#4ade80' }
                                : { label:'Excepcional', cor:'#00f0ff' };

                            const gnTiers = {
                                'Normal':    { cor:'#6b7280', icon:'âš”ï¸', beneficio: 'EvoluÃ§Ã£o padrÃ£o do sistema.' },
                                'Talentoso': { cor:'#60a5fa', icon:'ðŸ’ ', beneficio: '+2 Graus de PotÃªncia no Primeiro Hatsu.' },
                                'GÃªnio':     { cor:'#fbbf24', icon:'âœ¨', beneficio: 'XP 1,5Ã— em qualquer situaÃ§Ã£o.' },
                                'Ultimate':  { cor:'#ff4df7', icon:'ðŸ‘‘', beneficio: 'XP dobrado + +5 Graus de PotÃªncia livres.' },
                                // retrocompatibilidade com nomes antigos
                                'Ã“timo':     { cor:'#6b7280', icon:'âš”ï¸', beneficio: 'EvoluÃ§Ã£o padrÃ£o do sistema.' },
                                'Excelente': { cor:'#a78bfa', icon:'ðŸ’œ', beneficio: '+2 Graus de PotÃªncia no Primeiro Hatsu.' },
                            };
                            const gnData = gnTiers[gn] || gnTiers['Normal'];
                            const isSpecial = gn === 'Talentoso' || gn === 'GÃªnio' || gn === 'Ultimate' || gn === 'Excelente';
                            ...
                            const benefitBlock = isSpecial ? `
                                <div style="margin-top:8px;background:#0d1117;border:1px solid ${gnData.cor}44;border-radius:8px;padding:8px;text-align:left">
                                    <div style="font-size:8px;font-weight:900;color:${gnData.cor};text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">âš¡ BENEFÃCIO</div>
                            ...
                            return `<div style="width:100%;margin-bottom:16px;padding:0 4px">
                                <div style="background:#0a0f1a;border:2px solid ${gnData.cor}66;border-radius:12px;padding:14px;text-align:center">
                                    <div style="font-size:7px;font-weight:900;color:#6b7280;text-transform:uppercase;letter-spacing:2px;margin-bottom:6px">ðŸŽ¯ Talento para ProficiÃªncia de NEN</div>
                            ...
                            return `<div style="width:100%;padding:0 4px;margin-bottom:14px">
                                <div style="background:#0a0f1a;border:1px solid #1f2937;border-radius:12px;padding:12px">
                                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                                        <div style="font-size:8px;font-weight:900;color:#6b7280;text-transform:uppercase;letter-spacing:2px">âš¡ Pool de P.N do Personagem</div>
                                    ...
                                    <div style="display:flex;justify-content:space-between">
                                        <span style="font-size:8px;color:#6b7280">${pnUsedAll} P.N Hatsus Â· ${pnDominio} P.N DomÃ­nio</span>
                                    ...
                                    <div style="margin-top:8px;padding-top:8px;border-top:1px solid #1f2937;display:flex;justify-content:space-between;align-items:center">
                                        <span style="font-size:8px;color:#60a5fa;font-weight:700">ðŸ’  Graus de PotÃªncia livres</span>
                        ...
                        ${(() => {
                            // â”€â”€ DOMÃNIO DE NEN SECTION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                            const pnTotal = window.calcularPHBase ? window.calcularPHBase(char.level) : 6;
                            const pnDominio = window.calcPNSpentInDominio ? window.calcPNSpentInDominio(char) : 0;
                            ...
                            const fundamentalCard = (key, label, icon, desc, levelsData, reqCheck) => {
                                ...
                                const canDown = cur > 0;
                                const levelLabel = ['â€”','BÃ¡sico','IntermediÃ¡rio','Maestria'][cur];
                                ...
                                return `<div style="background:#0a0f1a;border:1.5px solid ${cur>0?tc2+'66':'#1f2937'};border-radius:12px;padding:12px;margin-bottom:8px">
                                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
                                        ...
                                        <div style="display:flex;align-items:center;gap:6px">
                                            <button onclick="${setDom(key, Math.max(0, cur-1))}" ${canDown?'':'disabled'}
                                                style="width:24px;height:24px;border-radius:6px;background:${canDown?'#1f2937':'#111'};border:1px solid #374151;color:${canDown?'#f87171':'#374151'};font-size:14px;cursor:${canDown?'pointer':'not-allowed'};display:flex;align-items:center;justify-content:center">âˆ’</button>
                                    ...
                                    ${cur > 0 ? `<div style="font-size:8px;font-weight:700;color:${levelColor};margin-bottom:4px">â— ${levelLabel}</div>
                                    <div style="font-size:8px;color:#9ca3af;line-height:1.5">${levelsData[cur-1]}</div>` : `<div style="font-size:8px;color:#374151;font-style:italic">NÃ£o desbloqueado (custa 1 P.N por nÃ­vel)</div>`}
                                </div>`;
                            };
                                    ...
                                    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
                                        <div style="flex:1">
                                            <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
                                                ...
                                                ${active?`<span style="font-size:7px;font-weight:900;padding:1px 5px;background:#a78bfa22;color:#a78bfa;border-radius:4px">âœ“ ATIVO</span>`:''}
                                            </div>
                                        ...
                                        <button onclick="${active?setDom(key, false):setDom(key, true)}" ${(canBuy||active)?'':'disabled'}
                                            ...
                                            ${active?'âˆ’1 P.N':'+1 P.N'}
                                        </button>
                                    </div>
                                </div>`;
                            };

                            const isReforco = char.class === 'REFORÃ‡O' || char.class === 'INTENSIFICAÃ‡ÃƒO';
                            const tenLevels = isReforco ? [
                                '10% Aura + 1 ReaÃ§Ã£o â†’ +2 RD (Corte, Impacto, ExplosÃ£o). Bloqueia intimidaÃ§Ã£o por aura.',
                                '5% Aura + 1 ReaÃ§Ã£o â†’ +4 RD. Imune a proj. que igualem CA.',
                                '10% Aura + 1 ReaÃ§Ã£o â†’ +6 RD. Desbloqueia SHU.'
                            ] : [
                                '20% Aura + 1 ReaÃ§Ã£o â†’ +2 RD (Corte, Impacto, ExplosÃ£o). Bloqueia intimidaÃ§Ã£o por aura.',
                                '20% Aura + 1 ReaÃ§Ã£o â†’ +4 RD. Imune a proj. que igualem CA.',
                                '20% Aura + 1 ReaÃ§Ã£o â†’ +6 RD. Desbloqueia SHU.'
                            ];
                            const renLevels = [
                                '5% Aura + AÃ§Ã£o BÃ´nus â†’ +1 Grau/Passo de dano, interaÃ§Ã£o com NEN, proteÃ§Ã£o vs REN.',
                                '10% Aura + AÃ§Ã£o BÃ´nus â†’ BÃ¡sico + +3 em IntimidaÃ§Ã£o/Arcanismo com REN. Desbloqueia EN.',
                                '10% Aura + AÃ§Ã£o BÃ´nus â†’ IntermediÃ¡rio + 1Ã—/dia +6 IntimidaÃ§Ã£o sem gastar aura. Desbloqueia GYO.'
                            ];
                            const zetsuLevels = [
                                '3 rodadas â†’ +5% Aura, +1 ReaÃ§Ã£o, +3 Furtividade. (Dano crÃ­tico ao sofrer NEN)',
                                '2 rodadas â†’ +10% Aura, +1 ReaÃ§Ã£o, +3 Furtividade. Desbloqueia IN.',
                                '1 rodada â†’ +10% Aura, +2 ReaÃ§Ãµes, +6 Furtividade. Criaturas nÃ£o-inteligentes te ignoram.'
                            ];

                            const ten = d.ten||0; const ren = d.ren||0; const zetsu = d.zetsu||0;

                            return `<div style="width:100%;padding:0 4px;margin-bottom:16px">
                                <div style="font-size:9px;font-weight:900;color:#6b7280;text-transform:uppercase;letter-spacing:2px;margin-bottom:10px;text-align:center">ðŸ”® DomÃ­nio de NEN</div>
                                ...
                                <div style="background:#060d1a;border:1px solid #1f2937;border-radius:10px;padding:8px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center">
                                    <span style="font-size:8px;color:#6b7280;font-weight:700">P.N em DomÃ­nio</span>
                                    <span style="font-family:'Orbitron',sans-serif;font-weight:900;font-size:12px;color:${tc2}">${pnDominio} / ${pnTotal}</span>
                                </div>

                                <div style="font-size:8px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">âš¡ PRINCÃPIOS FUNDAMENTAIS</div>
                                ${fundamentalCard('ten','TEN â€” ProteÃ§Ã£o','ðŸ›¡ï¸','Envolver/Proteger. Ativa RD contra golpes fÃ­sicos.',tenLevels,null)}
                                ${fundamentalCard('ren','REN â€” Controlar','ðŸ’ª','Praticar/Expandir. Aumenta grau de dano e intimidaÃ§Ã£o.',renLevels,null)}
                                ${fundamentalCard('zetsu','ZETSU â€” Suprimir','ðŸ‘ï¸','Anular/Ocultar. Recupera aura e oculta presenÃ§a.',zetsuLevels,null)}

                                <div style="font-size:8px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:1px;margin:10px 0 6px">ðŸŒŸ PRINCÃPIOS AVANÃ‡ADOS</div>
                                ${advancedCard('en','EN â€” Envolver','ðŸ”µ','ExpansÃ£o do Ren em esfera de detecÃ§Ã£o (3m+). 10% Aura + 2 ReaÃ§Ãµes.',
                                    '10% Aura + 2 ReaÃ§Ãµes â†’ detecta tudo a 3m. Ataque de reaÃ§Ã£o contra quem entrar.',
                                    'REN 2 + TEN 1', ren>=2 && ten>=1)}
                                ${advancedCard('inp','IN â€” Ocultar','ðŸŒ‘','Oculta objetos de aura. 5% Aura por rodada.',
                                    '5% Aura â†’ oculta item de aura atÃ© prÃ³ximo turno. Com Zetsu Maestria: +1 rodada ou âˆ’5%.',
                                    'ZETSU 2', zetsu>=2)}
                                ${advancedCard('gyo','GYO â€” Focar','ðŸ‘ï¸','Concentra aura nos olhos/corpo. 10%/30% Aura.',
                                    '10% Aura â†’ ver aura e oculto. 30% Aura â†’ +3 FOR/DES/CON por 1 rodada.',
                                    'REN Maestria + ZETSU 2', ren>=3 && zetsu>=2)}
                                ${advancedCard('shu','SHU â€” Agregar','âš¡','Envolve objetos em aura. 10% Aura.',
                                    '10% Aura â†’ +1d4 Dano e CA em objeto (1m). Golpe considerado mirado sem Ã´nus.',
                                    'TEN Maestria', ten>=3)}

                                <div style="font-size:8px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:1px;margin:10px 0 6px">ðŸ”± TÃ‰CNICAS AVANÃ‡ADAS</div>
                                ${advancedCard('ken','KEN â€” Fortificar','ðŸ°','Combina REN+TEN em proteÃ§Ã£o total. 30% Aura + 4 ReaÃ§Ãµes.',
                                    '30% Aura + 4 ReaÃ§Ãµes â†’ CA dobrada por 2 rodadas. Pode triplicar gastando o dobro.',
                                    'TEN Maestria + REN Maestria', ten>=3 && ren>=3)}
                                ${advancedCard('ko','KO â€” Endurecer','ðŸ”¥','Toda aura em um ponto. 30% Aura â†’ dano Ã—3, CA âˆ’80%.',
                                    '30% Aura â†’ triplica dano, mas CA cai 80% atÃ© prÃ³ximo turno.',
                                    'REN Maestria + ZETSU Maestria + TEN 1', ren>=3 && zetsu>=3 && ten>=1)}
                                ${advancedCard('ryu','RYU â€” Fluir','ðŸŒŠ','Maestria total em tempo real. 30% Aura.',
                                    '30% Aura â†’ mÃºltiplos contra-ataques, +3 CA e Ataque por 3 turnos.',
                ...
                const raceData = SYSTEM_DB.racas.find(r => r.nome === char.race); const bgData = SYSTEM_DB.antecedentes.find(b => b.nome === char.background); let traitsHtml = `<div class="flex items-center gap-3 mb-2 mt-2"><div class="bg-gray-800 p-2 rounded text-[${themeColor}]"><i data-lucide="dna" size="20"></i></div><div><h3 class="font-bold text-white uppercase tracking-wider text-sm">${char.race}</h3><p class="text-[10px] text-gray-500 uppercase tracking-widest">CaracterÃ­sticas Raciais</p></div></div><div class="space-y-2 mb-6">`; 
                
                // Add Origin Display Here
                if (char.race === 'Formiga Quimera' && char.fagogenese) {
                    traitsHtml += `<div class="bg-gray-900 border border-gray-800 p-3 rounded-xl border-l-2 border-l-[${themeColor}] mb-2"><h4 class="font-bold text-white text-xs mb-1">Origem da FagogÃªnese</h4><p class="text-[10px] text-gray-400 leading-relaxed uppercase tracking-widest font-bold text-neon-blue">${char.fagogenese}</p></div>`;
                }
                ...
                if (!isShop) {
                    ...
                    const categories = [ { id: 'armas', label: 'Armas', icon: 'sword' }, { id: 'armaduras', label: 'Armaduras', icon: 'shield' }, { id: 'municoes', label: 'MuniÃ§Ãµes', icon: 'crosshair' }, { id: 'itens_medicos', label: 'MÃ©dicos', icon: 'heart' }, { id: 'kits', label: 'Kits', icon: 'briefcase' }, { id: 'equipamentos_gerais', label: 'Gerais', icon: 'backpack' } ];
                ...
                // â”€â”€ Sub-aba HISTÃ“RICO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                const _historicoHtml = `
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
                        <h3 style="font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:.1em;font-size:11px">HistÃ³rico de Rolagens</h3>
                ...
                // â”€â”€ Sub-aba DISCORD â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                            ...
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">
                                <div style="flex:1;min-width:0">
                                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
                                        ${isDefault ? '<span style="font-size:9px;background:#92400e;color:#fbbf24;border-radius:4px;padding:2px 6px;font-weight:900;text-transform:uppercase;letter-spacing:.05em">â­ PadrÃ£o</span>' : ''}
                                        <span style="font-weight:900;color:#fff;font-size:12px">${w.name}</span>
                                    </div>
                                    <div style="font-size:9px;color:#4b5563;font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px">${w.url.replace(/^https:\/\/discord\.com\/api\/webhooks\//, 'discord.com/â€¦/')}</div>
                                </div>
                                <div style="display:flex;gap:6px;flex-shrink:0">
                                    <button onclick="setDefaultWebhook(${i})" title="${isDefault ? 'Remover padrÃ£o' : 'Marcar como padrÃ£o'}" style="padding:6px 8px;border-radius:8px;border:2px solid ${isDefault ? '#f59e0b' : '#374151'};background:${isDefault ? '#92400e' : 'transparent'};color:${isDefault ? '#fbbf24' : '#6b7280'};cursor:pointer;font-size:13px;line-height:1">â­</button>
                                    ${state.isAdmin ? `<button onclick="deleteWebhook(${i})" title="Apagar webhook" style="padding:6px 8px;border-radius:8px;border:2px solid #7f1d1d;background:transparent;color:#f87171;cursor:pointer;font-size:13px;line-height:1">ðŸ—‘</button>` : ''}
                                </div>
                            </div>
                        </div>`;
                    }).join('');
                ...
                const _statusHtml = `<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:10px;padding:10px 14px;margin-bottom:16px;font-size:10px">
                    <div style="color:#60a5fa;font-weight:900;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px">ðŸ“¡ Status</div>
                    <div style="color:#94a3b8">${_defWh >= 0 && _whs[_defWh] ? `PadrÃ£o ativo: <strong style="color:#fff">${_whs[_defWh].name}</strong> â€” rolagens enviadas direto.` : _whs.length > 1 ? 'Nenhum padrÃ£o â€” vocÃª escolherÃ¡ o destino a cada rolagem.' : _whs.length === 1 ? `Usando <strong style="color:#fff">${_whs[0].name}</strong> automaticamente.` : 'Nenhum webhook configurado.'}</div>
                </div>`;
                ...
                const catColors = { 'CURTA DURAÃ‡ÃƒO':'#fbbf24', 'LONGA DURAÃ‡ÃƒO':'#f87171', 'PERMANENTE LEVE':'#a78bfa', 'PERMANENTE PESADO':'#ff4df7' };

                const activeHtml = activeConditions.length === 0
                    ? '<div class="text-center text-gray-600 italic py-6 border-2 border-dashed border-gray-800 rounded-xl text-xs">Nenhuma condiÃ§Ã£o ativa.<br><span class="text-gray-700">Reduza a Sanidade para registrar surtos.</span></div>'
                    : activeConditions.map(function(c) {
                        ...
                        return '<div style="border:2px solid '+cc+'" class="bg-gray-900 rounded-xl p-4 relative mb-3">'
                            ...
                            + '<div class="text-[9px] text-gray-500 mt-0.5">'+c.data+' â€¢ Sanidade '+c.sanPct+'%</div></div>'
                            + '<button onclick="window._resolveCondition('+idx+')" class="text-[9px] text-green-500 border border-green-500/40 px-2 py-1 rounded hover:bg-green-500/10 font-bold uppercase">âœ“ Resolver</button>'
                            + '</div>'
                            + '<div class="text-sm text-white font-semibold leading-relaxed mb-2">'+c.condicao+'</div>'
                            + '<div class="flex gap-2 flex-wrap">'
                            + '<span class="text-[9px] bg-gray-800 border border-gray-700 px-2 py-0.5 rounded font-mono">ðŸŽ² '+c.roll+'</span>'
                            + '<span style="color:'+cc+'" class="text-[9px] bg-gray-800 border border-gray-700 px-2 py-0.5 rounded font-bold">â± '+c.duracao+'</span>'
                            + '</div></div>';
                    }).join('');
                    ...
                    ? '<div class="border-t border-gray-800 pt-3 mt-3"><div class="text-[10px] font-bold text-gray-600 uppercase tracking-wider mb-2">Resolvidas</div>'
                        + resolvedConditions.map(function(c) {
                            ...
                            return '<div class="bg-gray-900/50 border border-gray-800 rounded-xl p-3 opacity-50 mb-2">'
                                ...
                                + '<div class="text-[9px] text-gray-600 mt-1">â± '+c.duracao+'</div>'
                                + '</div>';
                        }).join('')
                        + '</div>'
                    : '';
                ...
                tabContent = `<div class="p-4 h-full flex flex-col">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="font-bold text-white uppercase tracking-wider text-xs flex items-center gap-2"><i data-lucide="brain" size="14"></i> CondiÃ§Ãµes de Sanidade</h3>
                    ...
                    <div class="bg-gray-900 border border-gray-800 rounded-xl p-3 mb-4">
                        ...
                        ${rdmVal > 0 ? '<div class="mt-2 text-center text-[10px] font-bold text-blue-400">ðŸ›¡ï¸ RDM '+rdmVal+' â€” reduz dano mental recebido</div>' : ''}
                    </div>
                        ...
                        <div class="absolute bottom-0 left-0 w-full p-6 flex flex-col justify-end z-10" onclick="event.stopPropagation()">
                            ...
                            <div class="flex items-center gap-4 w-full">
                                ...
                                <div class="flex items-center gap-2 bg-black/60 backdrop-blur rounded-lg px-2 py-1 border border-white/10">
                                    ...
                                    <div class="flex flex-col items-center leading-none px-1"><span class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">NÃ­vel</span><span class="text-xl font-display font-bold text-white">${char.level}</span></div>
        ...
        function findItemData(itemName) {
            ...
            const foundMun = ITEM_DB.municoes.find(i => i.nome.toLowerCase() === cleanName); if(foundMun) return { ...foundMun, category: 'MuniÃ§Ãµes' };
            const foundMed = ITEM_DB.itens_medicos.find(i => i.nome.toLowerCase() === cleanName); if(foundMed) return { ...foundMed, category: 'MÃ©dicos' };
        ...
        window.rollAfinidadeGenialidade = function() {
            ...
            // Talento para ProficiÃªncia de NEN: 2d20 (2-40)
            const d1 = Math.floor(Math.random() * 20) + 1;
            const d2 = Math.floor(Math.random() * 20) + 1;
            const genRoll = d1 + d2;
            let genialidade;
            if (genRoll === 40)       genialidade = 'Ultimate';  // 0.25%
            else if (genRoll >= 37)   genialidade = 'GÃªnio';     // ~2.5%
        ...
        function toggleRaceTrait(traitName) { const list = state.tempChar.raceTraits || []; const idx = list.indexOf(traitName); if (idx >= 0) { list.splice(idx, 1); if(state.tempChar.traitDetails && state.tempChar.traitDetails[traitName]) { delete state.tempChar.traitDetails[traitName]; } } else { if (list.length >= 3) { alert("VocÃª sÃ³ pode escolher atÃ© 3 caracterÃ­sticas."); return; } list.push(traitName); } state.tempChar.raceTraits = list; render(true); }
        ...
        // â”€â”€ Gerenciamento de Webhooks do Discord â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        function initWebhooks() {
            // Migra o webhook original caso ainda nÃ£o haja nada salvo
        ...
        function rollHatsuDamage(mode) {
            ...
            if (!rs || !rs.dice || !rs.hasBaseDmg) { alert('Este Hatsu nÃ£o possui dano calculÃ¡vel.'); return; }
            const char = state.currentChar;
            ...
            let flatBonus = 0;
            const flatParts = [];   // ex: ['+2', '+3'] â†’ para exibir na fÃ³rmula
            ...
            (rs.dmgExtras || []).forEach(e => {
                if (e.dado === '(contÃ­nuo)') return;
                const rollable = /^\+?\d+d\d+$/.test(e.dado);
                if (rollable) {
                    const er = rollDiceExpr(e.dado.replace(/^\+/, ''));
                    extraLines.push(`ðŸ’¥ ${e.tipo}: [${er.rolls.join('+')}] = **${er.total}**`);
                } else {
            ...
            char.history.push({ time, label: `âš¡ ${rs.nome} (${rs.attr})${modeLabel}`, dice: dmgResult.rolls.join(', '), mod: mod + flatBonus, total });
            if (char.history.length > 50) char.history.shift();
            saveCharacter(char);

            // FÃ³rmula: "2d10 + PRE + 5" com os bÃ´nus fixos somados
            ...
            // â”€â”€ Rolagem de Ataque â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            let attackLine = '';
            if (rs.hasAttack) {
                const nivel = rs.nivel || parseInt(char.level) || 1;
                const pb = getProficiencyBonus(nivel);
                // Segredo Mortal: forÃ§a vantagem se modo for Normal
                ...
                const critSuffix = isCrit ? ' ðŸŽ¯ **CRÃTICO!**' : isFumble ? ' ðŸ’€ **FALHA CRÃTICA!**' : '';
            ...
            // â”€â”€ Fim rolagem de ataque â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

            const content = `âš¡ **${char.name}** usou **${rs.nome}**${modeLabel}${attackLine}\nDano: [${dmgResult.rolls.join('+')}]${allModStr} = **${total}** (${formula})${extrasText}`;
        ...
        function executePendingRoll(mode) {
            ...
            state.rollMode = mode; // persiste o Ãºltimo modo usado
            if (!pr) return;
        ...
        function renderRollModeModalHtml() {
            ...
            const MODES = [
                { id: 'NORMAL',      label: 'Normal',      icon: 'ðŸŽ²', desc: '1d20',        bg: '#1f2937', col: '#9ca3af' },
                { id: 'VANTAGEM',    label: 'Vantagem',    icon: 'â¬†ï¸', desc: '2d20 maior',  bg: '#14532d', col: '#4ade80' },
                { id: 'DESVANTAGEM', label: 'Desvantagem', icon: 'â¬‡ï¸', desc: '2d20 menor',  bg: '#7f1d1d', col: '#f87171' },
                { id: 'ÃŠNFASE',      label: 'ÃŠnfase',      icon: 'ðŸŽ¯', desc: '2d20 â‰ˆ 10',   bg: '#1e1b4b', col: '#a78bfa' },
            ];
            const _mwhs = loadWebhooks();
            // SÃ³ mostra o seletor se hÃ¡ mÃºltiplos webhooks E nenhum padrÃ£o definido
            const _showWhPicker = _mwhs.length > 1 && state.defaultWebhook < 0;
            const _whPickerHtml = _showWhPicker ? `
                <div style="margin-bottom:14px">
                    <div style="font-size:9px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px;text-align:center">ðŸ“¡ Enviar para</div>
            ...
            return `<div class="fixed inset-0 z-[200] flex items-end justify-center" style="background:rgba(0,0,0,0.75);backdrop-filter:blur(4px)" onclick="cancelPendingRoll()">
                <div style="background:#0d1117;border-radius:20px 20px 0 0;border:1px solid #1f2937;border-bottom:none;padding:20px 16px 32px;width:100%;max-width:440px" onclick="event.stopPropagation()">
                    ...
                    ${_whPickerHtml}
                    <div style="font-size:9px;font-weight:900;color:#4b5563;text-transform:uppercase;letter-spacing:2px;margin-bottom:14px;text-align:center">ðŸŽ² Modo de Rolagem</div>
        ...
        function rollAttack(weaponName, diceExpr, attrKey, mode) {
            ...
            const time = new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
            char.history.push({ time, label: `âš”ï¸ ${weaponName} (${attrKey})`, dice: attackRoll.dice.join(', '), mod: mod + pb, total: attackTotal });
            if (char.history.length > 50) char.history.shift();
            saveCharacter(char);
            const isCrit = attackRoll.dice.includes(20);
            const isFumble = attackRoll.dice.length === 1 && attackRoll.dice[0] === 1;
            const suffix = isCrit ? ' ðŸŽ¯ **CRÃTICO!**' : isFumble ? ' ðŸ’€ **FALHA CRÃTICA!**' : '';
            const content = `âš”ï¸ **${char.name}** atacou com **${weaponName}**\nAtaque: [${attackRoll.dice.join(', ')}] +${mod + pb} = **${attackTotal}**${suffix}\nDano: [${dmgResult.rolls.join('+')}] +${mod} = **${dmgTotal}** (${diceExpr} + ${attrKey})`;
            fetch(getActiveWebhookUrl(), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content }) }).catch(() => {});
            state.attackModal = null;
            state.rollResult = { name: `âš”ï¸ ${weaponName}`, total: attackTotal, diceVal: attackRoll.total, mod: mod + pb, label: attackRoll.label };
            if (state.activeTab !== 'DADOS') state.unreadRolls = true;
            render(true);
        }
        function sendToDiscord(label, dice, mod, total, charName) {
            ...
            let suffix = isCrit ? ' ðŸŽ¯ **CRÃTICO!**' : isFumble ? ' ðŸ’€ **FALHA CRÃTICA!**' : '';
            const content = `ðŸŽ² **${charName}** rolou **${label}**\nDado: ${diceStr}${modStr} = **${total}**${suffix}`;
            fetch(getActiveWebhookUrl(), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content }) }).catch(() => {});
        }
        ...
        // â”€â”€ Dado de vida por categoria â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const HIT_DICE = {
            'INTENSIFICAÃ‡ÃƒO': { dado: 'd12', faces: 12, media: 7 },
            'REFORÃ‡O':         { dado: 'd12', faces: 12, media: 7 },
            'TRANSMUTAÃ‡ÃƒO':    { dado: 'd10', faces: 10, media: 6 },
            'EMISSÃƒO':         { dado: 'd10', faces: 10, media: 6 },
            'MATERIALIZAÃ‡ÃƒO':  { dado: 'd8',  faces: 8,  media: 5 },
            'MANIPULAÃ‡ÃƒO':     { dado: 'd8',  faces: 8,  media: 5 },
            'ESPECIALIZAÃ‡ÃƒO':  { dado: 'd6',  faces: 6,  media: 4 },
        };

        // â”€â”€ Abre modal de ganho de XP â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        window.openXpModal = function() {
            ...
            overlay.innerHTML = `
                <div style="background:#0d1117;border:2px solid ${tc};border-radius:16px;padding:24px;width:100%;max-width:360px;box-shadow:0 0 40px ${tc}33">
                    <div style="font-family:Orbitron,sans-serif;font-weight:900;font-size:13px;color:${tc};text-transform:uppercase;letter-spacing:2px;margin-bottom:4px">âœ¨ Adicionar XP</div>
                    <div style="font-size:10px;color:#6b7280;margin-bottom:16px">XP atual: <b style="color:#d1d5db">${curXp}</b> / <b style="color:${tc}">${nextXp}</b> para o prÃ³ximo nÃ­vel</div>
                    <!-- barra atual -->
        ...
        window.confirmXpGain = function() {
            ...
            // Talento para ProficiÃªncia de NEN: multiplicadores de XP
            const genTier = char.genialidade;
            const xpMultiplier = genTier === 'Ultimate' ? 2 : (genTier === 'GÃªnio' ? 1.5 : 1);
            ...
            // Verifica se passou de nÃ­vel (pode passar mais de um)
            ...
            if (newLevel > char.level) {
                ...
                saveCharacter(char);
                render(true);
                if (xpMultiplier > 1) window._showXpToast(`+${finalGained} XP ${xpMultiplier === 2 ? 'ðŸ‘‘ (Ã—2 Ultimate)' : 'âœ¨ (Ã—1.5 GÃªnio)'}`);
        ...
        // â”€â”€ Modal de Level Up com dado de vida â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        window.openLevelUpModal = function(levelsGained = 1, isGenius = false, xpGained = 0) {
            ...
            overlay.innerHTML = `
                <div style="background:#0d1117;border:2px solid ${tc};border-radius:20px;padding:24px;width:100%;max-width:370px;box-shadow:0 0 60px ${tc}44;text-align:center">
                    ...
                    <div style="font-family:Orbitron,sans-serif;font-weight:900;font-size:16px;color:${tc};text-transform:uppercase;letter-spacing:3px;text-shadow:0 0 12px ${tc}88">NÃ­vel ${newLevel}!</div>
                    <div style="font-size:10px;color:#6b7280;margin-top:4px;margin-bottom:16px">${levelsGained > 1 ? `+${levelsGained} nÃ­veis ganhos de uma vez!` : 'VocÃª subiu de nÃ­vel!'}</div>
                    ${isGenius ? `<div style="font-size:9px;color:#fbbf24;background:#fbbf2411;border:1px solid #fbbf2433;border-radius:8px;padding:6px 12px;margin-bottom:14px">âœ¨ GÃªnio: +${xpGained} XP (1.5Ã—)</div>` : ''}
                    <!-- Dado de vida -->
                    <div style="background:#0a0f1a;border:1px solid #1f2937;border-radius:14px;padding:16px;margin-bottom:16px">
                        <div style="font-size:9px;font-weight:900;color:#6b7280;text-transform:uppercase;letter-spacing:2px;margin-bottom:10px">ðŸŽ² Dado de Vida â€” ${hitDiceData.dado} (${char.class})</div>
                        <div id="lv-dice-result" style="font-family:Orbitron,sans-serif;font-size:40px;font-weight:900;color:#fff;margin-bottom:4px;min-height:52px;line-height:1">â€”</div>
                    ...
                    ${calcRDM(char) > 0 ? `<div style="background:#0a0f1a;border:1px solid #1f293788;border-radius:10px;padding:10px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center">
                        <span style="font-size:9px;color:#9ca3af;font-weight:700">ðŸ§  RecuperaÃ§Ã£o de Sanidade</span>
                        <span style="font-family:Orbitron,sans-serif;font-weight:900;font-size:14px;color:#c084fc">+${calcRDM(char)}</span>
                    </div>` : ''}
                    <!-- BotÃ£o confirmar (desabilitado atÃ© rolar/escolher) -->
            ...
            window._lvPickMedia = function() {
                ...
                let breakdown = `${hitDiceData.media} (mÃ©dia) + ${conMod >= 0?'+':''}${conMod} CON`;
                if (giantBonus) breakdown += ` + ${giantBonus} Gigante`;
                document.getElementById('lv-dice-breakdown').textContent = breakdown;
                window._lvEnableConfirm();
            };
            window._lvEnableConfirm = function() {
                ...
                // Desabilita os botÃµes de rolar/mÃ©dia
            ...
            window._lvConfirm = function() {
                ...
                const sanRecovery = newRdm; // recupera o RDM do novo nÃ­vel
                ...
                render(true);
                window._showXpToast('NÃ­vel ' + newLevel + '! +' + gain + ' PV' + (sanRecovery > 0 ? ' | +' + sanRecovery + ' Sanidade' : ''));
            };
        };
        ...
        function changeLevel(delta) {
            ...
            if (delta > 0) {
                ...
                if (!confirm('Reduzir nÃ­vel nÃ£o desfaz automaticamente os ganhos de vida. Continuar?')) return;
                char.level = newLevel;
                // Ajusta xp_next para o nÃ­vel anterior
        ...
        function clearHistory() { if(confirm('Limpar histÃ³rico?')) { state.currentChar.history = []; saveCharacter(state.currentChar); render(true); } }
...
//  renderHatsuDetail â€” tela de visualizaÃ§Ã£o do Hatsu
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
